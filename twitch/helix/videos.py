        ...
        if video_ids:
            ...
            if api.use_cache:
                ...
                for video_id in list(kwargs['id']):
                    ...
                    if cache_data:
                        self._data.append(helix.Video(api=api, data=cache_data))
            ...
            # Download from API
            if len(kwargs['id']):
                for data in api.get(self._path, params=kwargs, ignore_cache=True)['data']:

                    # Create and append user
                    video = helix.Video(api=api, data=data)
